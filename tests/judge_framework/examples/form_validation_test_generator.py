...
CONCEPT_TYPES = {
    "Date": {
        ...
        "common_violations": ["Date field used for numeric data", "Missing date validation"]
    },
    "DateTime": {
    ...
    "SingleSelect": {
        ...
        "invalid_patterns": ["Multi-select as SingleSelect", "Open text as SingleSelect"],
        "common_violations": ["Binary questions as MultiSelect", "Insufficient options"]
    },
    "MultiSelect": {
        "description": "Multiple selections from predefined options",
        "valid_patterns": ["Symptoms", "Multiple conditions", "Multiple services"],
        "invalid_patterns": ["Binary Yes/No as MultiSelect", "Single choice as MultiSelect"],
        "common_violations": ["Binary questions as MultiSelect", "Related but not multiple"]
    },
    "Image": {
    ...
    "Coded": {
        ...
        "common_violations": ["Subject dataType instead of Coded", "Missing answer options"]
    },
    "QuestionGroup": {
        "description": "Groups of related questions",
        "valid_patterns": ["Address Group", "Contact Information Group"],
        "invalid_patterns": ["Repeatable children as QuestionGroup", "Single field as Group"],
    ...
    "PhoneNumber": {
        ...
        "common_violations": ["Phone without validation", "International format issues"]
    },
    "Location": {
    ...
    critical_violations = [
        {
            ...
            "form_context": {"formType": "IndividualProfile", "domain": "health", "formName": "Patient Registration"},
            "expected_issues": ["CRITICAL: Name field should not be in IndividualProfile form"],
        ...
        },
        {
            ...
            "form_context": {"formType": "IndividualProfile", "domain": "health", "formName": "Patient Registration"},
            "expected_issues": ["HIGH: Age field using Text dataType instead of Numeric"],
            "test_priority": "CRITICAL"
        },
        {
        ...
        },
        {
            ...
            "form_context": {"formType": "IndividualProfile", "domain": "health", "formName": "Patient Registration"},
            "expected_issues": ["MEDIUM: Binary question should use SingleSelect instead of MultiSelect"],
            "test_priority": "CRITICAL"
        }
    ]
    ...
    high_priority_violations = [
        {
            ...
            "form_context": {"formType": "ProgramEncounter", "domain": "health", "formName": "Growth Monitoring"},
        ...
        },
        {
            ...
            "form_context": {"formType": "IndividualProfile", "domain": "health", "formName": "Patient Registration"},
            "expected_issues": ["MEDIUM: Subject dataType should only be used for relationships"],
            "test_priority": "HIGH"
        },
        {
            ...
            "form_context": {"formType": "HouseholdProfile", "domain": "health", "formName": "Household Registration"},
            "expected_issues": ["HIGH: Children should be separate subject types, not QuestionGroup"],
            "test_priority": "HIGH"
        }
    ]
    ...
    cancellation_tests = [
        {
            ...
            "expected_issues": ["HIGH: Cancellation reason should be mandatory in cancellation forms"],
            "test_priority": "HIGH",
        ...
        },
        {
            ...
            "expected_issues": ["MEDIUM: Cancellation reason should use Coded dataType with predefined options"],
    ...
    program_exit_tests = [
        {
            ...
            "expected_issues": ["HIGH: Exit reason should be mandatory in program exit forms"],
            "test_priority": "HIGH",
            "source": "new_form_type_analysis"
        },
        {
        ...
        },
        {
            ...
            "expected_issues": ["LOW: Follow-up plans should use Notes dataType for detailed text"],
            "test_priority": "LOW",
            "source": "new_form_type_analysis"
        }
    ]
...
def save_test_matrix(test_cases: List[Dict[str, Any]], filename: str) -> bool:
    ...
    try:
        ...
        output_dir = Path("/Users/himeshr/IdeaProjects/avni-ai/tests/judge_framework/test_suites/formElementValidation")
...
def update_comprehensive_matrix(new_test_cases: List[Dict[str, Any]]) -> bool:
    ...
    if not Path(comprehensive_matrix_file).exists():
        ...
        return save_test_matrix(new_test_cases, "comprehensive_form_validation_test_matrix.json")
...
def main():
    ...
    if args.mode == "comprehensive":
        ...
        success = save_test_matrix(test_cases, "comprehensive_form_validation_test_matrix.json")
    ...
    if success and test_cases:
        summary = {
            ...
            "form_types_covered": list(set(tc["form_context"]["formType"] for tc in test_cases)),
            "concept_types_covered": list(set(tc["form_element"]["concept"]["dataType"] for tc in test_cases)),
