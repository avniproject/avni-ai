...
def find_form_files(directory: Path) -> List[Path]:
    ...
    for root, dirs, files in os.walk(directory):
        dirs[:] = [d for d in dirs if not d.startswith('.') and d not in ['translations', 'extensions']]
...
def get_sample_forms(organization_dir: Path, sample_size: int = 8) -> List[Path]:
    ...
    samples.extend(random.sample(remaining, min(sample_size - len(samples), len(remaining))))
...
def analyze_form_elements(elements: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    known_concept_types = {
        'Date', 'DateTime', 'Text', 'Notes', 'Numeric', 'SingleSelect', 'MultiSelect',
        'Image', 'Video', 'Audio', 'File', 'Coded', 'QuestionGroup', 'PhoneNumber',
        'Location', 'Subject', 'GroupAffiliation', 'Encounter', 'NA', 'Time', 'Duration', 'Id', 'ImageV2'
    }
...
def analyze_field_name_patterns(field_names: List[str]) -> Dict[str, Any]:
    ...
    categories = {
        ...
        'infrastructure': ['building', 'facility', 'infrastructure', 'space', 'equipment'],
        'administrative': ['registration', 'approval', 'verification', 'certification', 'authorization']
    }
    ...
    return {
        'categorized_fields': {cat: fields[:5] for cat, fields in categorized_fields.items()},
...
def analyze_sampled_forms(sampled_forms: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    domain_patterns = {
        'education': ['school', 'anganwadi', 'teacher', 'student', 'assessment', 'capacity', 'training'],
        'logistics': ['distribution', 'dispatch', 'inventory', 'stock', 'supply', 'village'],
    ...
    return {
        ...
        'form_types_found': sorted(list(form_types)),
        'new_concept_types': sorted(list(new_concept_types)),
        'domain_patterns': {domain: sorted(list(patterns)) for domain, patterns in domain_keywords.items()},
