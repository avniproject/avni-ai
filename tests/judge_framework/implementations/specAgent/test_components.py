...
def test_entities_loading():
    ...
    # Validate structure
    expected_keys = ["subject_types", "programs", "encounter_types", "address_levels", "forms"]
    for key in expected_keys:
        if key not in entities:
            print(f"❌ Missing key in entities: {key}")
            return False
    
    print(f"✅ Entities loaded successfully")
...
def test_test_subject_creation():
    ...
    try:
        from tests.judge_framework.implementations.specAgent import SpecAgentTestSubjectFactory
        from tests.judge_framework.interfaces.result_models import (
            TestConfiguration, DifyConfig, EvaluationConfig, TestGenerationConfig
        )
        
        entities_file = str(project_root / "tests/resources/scoping/entities_summary.json")
        ...
        if "query" not in test_input or "inputs" not in test_input:
            print(f"❌ Invalid test input structure")
            return False
...
def test_scenarios_loading():
    ...
    scenarios_file = project_root / "tests/judge_framework/test_suites/specAgent/spec_agent_test_scenarios.json"
    ...
    if not scenarios:
        print(f"❌ No scenarios found in file")
        return False
...
def test_executor_creation():
    ...
    try:
        ...
        print(f"   - Supports variable injection: {metadata['supports_variable_injection']}")
...
def test_monitoring_tools():
    ...
    try:
        ...
        print(f"✅ Monitoring tools created successfully")
