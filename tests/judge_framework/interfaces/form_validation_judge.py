...
class FormValidationJudgeStrategy(ABC):
    ...
    def _call_openai_for_evaluation(self, evaluation_context: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"🔍 DEBUG - Evaluation context preview: {evaluation_context[:300]}...")
...
class DifyFormValidationJudgeStrategy(FormValidationJudgeStrategy):
    ...
    def evaluate(self, test_input: Dict[str, Any], test_output: Dict[str, Any]) -> EvaluationResult:
        ...
        if validation_feedback:
            print(f"🔍 DEBUG - validation_feedback preview: {validation_feedback[:200]}...")
        ...
        performance_score = self._calculate_performance_score(performance_metrics, test_input)
...
"""
        ...
        return EvaluationResult(
            ...
            execution_metadata={
                ...
                "validation_response_format": validation_details.get('response_format', 'unknown'),
            ...
            if response_text.startswith('['):
                ...
                for issue in parsed_array:
                    ...
                    if any(keyword in message for keyword in [
                        "correctly configured", "no issue", "valid", "properly configured",
                        "matches avni rules", "appropriately configured", "no violation"
                    ]):
                        continue  # Skip success messages
                    
                    # Check if this is a minor improvement suggestion (not a violation)
                    if any(keyword in message for keyword in [
                        "should configure bounds", "consider making mandatory", "should configure",
                        "recommend configuring", "consider adding", "ensure units are clear",
                        "you should configure", "configure appropriate"
                    ]):
    ...
    def _calculate_performance_score(self, performance_metrics: Dict[str, Any], test_input: Dict[str, Any]) -> float:
        ...
        ideal_threshold = performance_expectations.get("ideal_response_time_ms", 5000)  # Adjusted to 5s
        max_threshold = performance_expectations.get("max_response_time_ms", 10000)    # Adjusted to 10s
        ...
        if total_time_ms <= ideal_threshold:
            ...
            ratio = (total_time_ms - ideal_threshold) / (max_threshold - ideal_threshold)
