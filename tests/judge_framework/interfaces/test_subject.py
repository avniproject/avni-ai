...
class TestSubjectFactory(ABC):
    ...
    @abstractmethod
    def create_from_ai_generation(self, ai_prompt: str, config: TestConfiguration) -> 'TestSubject':
...
class TestSubjectGenerator:
    ...
    def generate_test_suite(self, config: TestConfiguration, num_ai_cases: int = 0) -> List[TestSubject]:
        """
    ...
    def _generate_ai_cases(self, config: TestConfiguration, num_cases: int) -> List[TestSubject]:
