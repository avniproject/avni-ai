...
class JudgeStrategy(ABC):
    ...
    def _calculate_overall_success(self, scores: Dict[str, float]) -> bool:
        ...
        for metric, score in scores.items():
            threshold = self.config.success_thresholds.get(metric, 75.0)  # Default 75% threshold
            if score < threshold:
                return False
        return True
...
"""
    ...
    def _get_evaluation_metrics(self) -> List[str]:
        return ["configuration_correctness", "consistency", "communication_quality", "task_completion"]
...
"""
    ...
    def _get_evaluation_metrics(self) -> List[str]:
        return ["validation_correctness", "edge_case_handling", "user_experience", "data_integrity"]
