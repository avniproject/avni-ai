...
class DifyFormValidationExecutor(FormValidationExecutor):
    ...
    def _initialize_dify_client(self):
        ...
        try:
            ...
            self.dify_client = DifyClient(api_key=self.config.dify_config.api_key)
        except ImportError:
            raise ImportError(
                "DifyClient not available. Please ensure tests.dify.common.dify_client is accessible."
            )
    ...
    def _format_form_validation_query(
        self, form_element: Dict[str, Any], form_context: Dict[str, Any]
    ) -> str:
    ...
    def _build_context_string(
        ...
        if "concept" in form_element:
            ...
            context_parts.append(
                f"Current dataType: {concept.get('dataType', 'Unknown')}"
            )
