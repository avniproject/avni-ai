...
class StatisticsCalculator:
    ...
    def calculate_suite_statistics(suite_result: TestSuiteResult) -> Dict[str, Any]:
        ...
        score_data = StatisticsCalculator._calculate_score_statistics(
            suite_result.individual_results
        )
        stats["score_statistics"] = score_data

        # Calculate performance metrics
        performance_data = StatisticsCalculator._calculate_performance_metrics(
            suite_result.individual_results
        )
        ...
        for metric, scores in metric_scores.items():
            if scores:
                score_stats[metric] = {
                    ...
                    "count": len(scores),
                    "pass_rate": len([s for s in scores if s >= 75])
                    / len(scores)
                    * 100,
                }
    ...
    def _calculate_performance_metrics(
        ...
        metrics["common_failure_patterns"] = [
            f"{category} ({count})" for category, count in sorted_errors[:5]
        ]
        ...
        for metric, scores in metric_variances.items():
            if len(scores) > 1:
                metrics["test_consistency"][metric] = {
                    ...
                    "coefficient_of_variation": statistics.stdev(scores)
