...
class ReportGenerator:
    ...
    def generate_console_report(
        suite_result: TestSuiteResult, statistics: Dict[str, Any] = None
    ) -> str:
        ...
        if statistics:
            ...
            if "score_statistics" in statistics:
                ...
                for metric, stats in statistics["score_statistics"].items():
                    ...
                    report.append(
                        f"       - Pass Rate (75+): {stats['pass_rate']:.1f}%"
                    )
        ...
        for i, result in enumerate(
            suite_result.individual_results[:10], 1
        ):  # Show first 10
            ...
            if result.scores:
                score_summary = ", ".join(
                    [f"{k}: {v:.0f}" for k, v in list(result.scores.items())[:3]]
                )
        ...
        if len(suite_result.individual_results) > 10:
            report.append(
                f"   ... and {len(suite_result.individual_results) - 10} more tests"
            )
    ...
    def generate_csv_report(suite_result: TestSuiteResult) -> str:
        ...
        # Write rows
        for result in suite_result.individual_results:
            row = [result.test_identifier, result.success, result.timestamp]
    ...
    def generate_comparison_report(
        suite_results: List[TestSuiteResult], comparison_stats: Dict[str, Any] = None
    ) -> str:
        ...
        report.append(
            f"{'Suite Type':<20} {'Total':<8} {'✅ Success':<10} {'❌ Failed':<8} {'📈 Rate':<8}"
        )
        report.append("-" * 60)

        for suite in suite_results:
            report.append(
                f"{suite.test_type:<20} {suite.total_tests:<8} "
