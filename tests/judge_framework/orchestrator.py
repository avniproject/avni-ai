...
from .interfaces.test_subject import TestSubject, TestSubjectFactory, TestSubjectGenerator
from .interfaces.test_executor import TestExecutor
from .interfaces.judge_strategy import JudgeStrategy
from .interfaces.result_models import EvaluationResult, TestSuiteResult, TestConfiguration
...
class ProgressReporter(Protocol):
    ...
    def on_test_complete(self, result: EvaluationResult, test_number: int, total_tests: int):
...
class JudgeOrchestrator:
    ...
    def run_test_suite(self, 
        ...
        self.progress_reporter.on_test_suite_start(suite_result.test_type, len(test_subjects))
        ...
        for i, test_subject in enumerate(test_subjects):
            ...
            self.progress_reporter.on_test_start(
                test_subject.get_test_identifier(), 
                test_number, 
                len(test_subjects)
            )
            ...
            self.progress_reporter.on_test_complete(result, test_number, len(test_subjects))
        
        # 5. Finalize suite result
        suite_result.end_time = datetime.now().isoformat()
        suite_result.success_rate = (suite_result.successful_tests / suite_result.total_tests * 100) if suite_result.total_tests > 0 else 0
    ...
    def _generate_test_subjects(self, 
                               factory: TestSubjectFactory, 
                               config: TestConfiguration) -> List[TestSubject]:
        """Generate test subjects using mixed approach"""
        generator = TestSubjectGenerator(factory)
        return generator.generate_test_suite(config, config.generation_config.num_ai_cases)
    ...
    def _calculate_suite_statistics(self, suite_result: TestSuiteResult):
        ...
        for metric in score_sums:
            suite_result.average_scores[metric] = score_sums[metric] / score_counts[metric]
        ...
        suite_result.common_errors = sorted(error_counts.keys(), key=lambda x: error_counts[x], reverse=True)
