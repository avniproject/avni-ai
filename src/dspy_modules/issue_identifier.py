...
class IssueIdentifier(dspy.Module):
    ...
    def forward(self, form_json: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                issues = json.loads(result.issues) if isinstance(result.issues, str) else result.issues
            ...
            return {
                "issues": [],
                "summary": f"Issue identification failed: {str(e)}"
            }
...
class AvniIssueIdentificationSignature(dspy.Signature):
    ...
    form_structure = dspy.InputField(desc="Complete Avni form JSON with formElementGroups, field configurations, and UUIDs")
    
    issues = dspy.OutputField(desc="JSON array of issues: [{\"category\": \"Data Type\", \"message\": \"Age field using Text dataType should use Numeric\", \"severity\": \"High\", \"formElementUuid\": \"uuid-123\", \"formElementName\": \"Age\", \"suggestedFix\": \"Change dataType from Text to Numeric with bounds 0-120\"}]")
    summary = dspy.OutputField(desc="Brief summary text of total issues found and main problems")