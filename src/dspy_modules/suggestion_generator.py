...
class SuggestionGenerator(dspy.Module):
    ...
    def forward(
        self, form_json: Dict[str, Any], issues: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            result = self.suggester(
                form_structure=form_str, identified_issues=issues_str
            )

            # Parse JSON outputs if they're strings
            try:
                suggestions = (
                    json.loads(result.suggestions)
...
class AvniSuggestionSignature(dspy.Signature):
    ...
    form_structure = dspy.InputField(
        desc="Complete Avni form JSON with formElementGroups and field configurations"
    )
    identified_issues = dspy.InputField(
        desc="JSON array of identified issues with severity, messages, and affected elements"
    )

    suggestions = dspy.OutputField(
        desc='JSON array of suggestions: [{"type": "Issue Fix", "title": "Fix Age dataType", "description": "Change Age field from Text to Numeric", "priority": "High", "formElementUuid": "uuid-123", "implementation": {"dataType": "Numeric", "lowAbsolute": 0, "highAbsolute": 120}, "rationale": "Age should be numeric for proper validation"}]'
    )
