...
class FormImprovementProgram(dspy.Module):
    ...
    def forward(self, form_json: Dict[str, Any], 
        ...
        try:
            ...
            executive_summary = {
                ...
                "overall_score": max(0, 100 - (critical_issues * 30) - (high_issues * 15) - (total_issues * 5))
            }
            ...
            logger.info(f"=== FormImprovementProgram.forward() Completed in {analysis_duration:.2f}s ===")
            logger.info(f"Final results: {total_issues} issues, {len(suggestions)} suggestions")
    ...
    async def analyze_multiple_forms(self, forms: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            "batch_analysis": {
                ...
                "failed_analyses": len([r for r in results if isinstance(r, Exception)])
            },
            "results": results
        }
