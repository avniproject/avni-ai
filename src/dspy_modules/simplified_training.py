    ...
    def avni_correctness_metric(example, pred, trace=None):
        ...
        try:
            ...
            if hasattr(example, 'issues') and hasattr(pred, 'issues'):
                example_issues = json.loads(example.issues) if isinstance(example.issues, str) else example.issues
                pred_issues = json.loads(pred.issues) if isinstance(pred.issues, str) else pred.issues
                ...
                if len(example_issues) == 0 and len(pred_issues) == 0:
                    ...
                    example_severities = [issue.get('severity', '') for issue in example_issues]
                    pred_severities = [issue.get('severity', '') for issue in pred_issues]
                    
                    critical_match = ('critical' in example_severities) == ('critical' in pred_severities)
                    if critical_match:
                        score += 0.3
...
async def train_simplified_analyzer(config: Dict[str, Any] = None) -> SimplifiedAvniAnalyzer:
