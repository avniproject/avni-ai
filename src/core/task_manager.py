...
# Context variable to store current task ID (thread-safe)
...
class TaskManager:
    ...
    def __init__(self, task_expiry_hours: int = 24):
        # Simple dict - no lock needed since UUIDs prevent collisions
    ...
    def create_task(
        ...
        task_id = str(uuid.uuid4())  # UUID guarantees uniqueness
        now = datetime.utcnow()
        ...
        # Direct assignment - no collision possible with UUID
    ...
    def update_task_status(
        ...
        task = self._tasks.get(task_id)
        if task:
            # Direct updates - only this background task touches this specific task
            task.status = status
    ...
    def start_background_task(self, task_id: str, processor, openai_api_key: str) -> None:
        ...
        background_task = asyncio.create_task(
            self._process_config_background(task_id, processor, openai_api_key),
    ...
    async def _process_config_background(
        self,
        task_id: str,
        processor,
        openai_api_key: str
        ...
        try:
            # Set current task ID in context for this background task
