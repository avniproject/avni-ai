...
class OpenAIResponsesClient:
    ...
    async def create_response(
        ...
        logger.info(f"📞 Making initial OpenAI API call with {len(formatted_tools)} tools")
    ...
    async def _make_continuation_call(
        ...
        logger.info(f"📝 Final input list length before continuation: {len(input_list)}")
        ...
        logger.info(f"✅ Got continuation response with {len(continue_response.output)} output items")
    ...
    async def process_function_calls_and_continue(
        ...
        logger.info(f"📝 Added response output to input_list. New length: {len(input_list)}")
        ...
        try:
            ...
            formatted_tools = format_tools_for_continuation(available_tools, tool_registry)
