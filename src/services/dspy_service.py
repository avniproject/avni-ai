...
class DSPyService:
        ...
        try:
            ...
            if check_trained_model_exists(model_path):
                ...
                try:
                    ...
                    logger.info("💡 To retrain models, run: python train_avni_models.py")
                ...
                if self.config.get("enable_fallback_training", False):
                    ...
                    try:
                        self.avni_analyzer = await train_simplified_analyzer(training_config)
    ...
    async def _configure_dspy(self):
        ...
        try:
            ...
            logger.info(f"DSPy configured with {self.config['model_provider']}/{self.config['model_name']}")
            
        except Exception as e:
            logger.error(f"Failed to configure DSPy: {e}")
            raise
    
    async def analyze_form(self, form_json: Dict[str, Any], 
        ...
        try:
            logger.info(f"Starting form analysis for: {form_json.get('name', 'Unknown')}")
            ...
            logger.info(f"Results structure before post-processing: {type(results)} with keys: {list(results.keys()) if isinstance(results, dict) else 'not a dict'}")
            results = self._post_process_results(results, form_json)
            
            logger.info(f"Form analysis completed. Results type: {type(results.get('ranked_recommendations', []))}")
    ...
    async def analyze_multiple_forms(self, forms: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    def _post_process_results(self, results: Dict[str, Any], 
                            original_form: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def optimize_model(self, training_data: List[Dict[str, Any]], 
        ...
        try:
            logger.info(f"Starting model optimization with {len(training_data)} examples")
