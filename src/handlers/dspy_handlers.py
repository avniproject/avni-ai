...
async def analyze_form_request(request: Request) -> JSONResponse:
    ...
    try:
        ...
        results = await dspy_service.analyze_form(
            form_json=form_json, analysis_options=analysis_options
        )
...
async def batch_analyze_forms_request(request: Request) -> JSONResponse:
    ...
    try:
        ...
        if not forms or not isinstance(forms, list):
            return JSONResponse(
                {"error": "forms must be a non-empty list"}, status_code=400
            )
        ...
        results = await dspy_service.analyze_multiple_forms(forms=forms)
        ...
        return JSONResponse(
            {"error": f"Batch analysis failed: {str(e)}"}, status_code=500
        )
...
async def predict_field_type_request(request: Request) -> JSONResponse:
    ...
    try:
        ...
        if not question_text:
            return JSONResponse({"error": "question_text is required"}, status_code=400)
        ...
        if predictions:
            ...
            return JSONResponse(
                {"success": False, "error": "No type prediction generated"}
            )
...
async def validate_form_structure_request(request: Request) -> JSONResponse:
    ...
    try:
        ...
        # Validate required fields
        if not form_json:
            return JSONResponse({"error": "form_json is required"}, status_code=400)
        ...
        return JSONResponse(
            {
                ...
                "data": {
                    ...
                    "overall_score": validation_results.get("summary", {}).get(
                        "overall_score", 0
                    ),
                },
            }
...
async def get_dspy_service_status_request(request: Request) -> JSONResponse:
    ...
    try:
        ...
        return JSONResponse(
            {"error": f"Status check failed: {str(e)}"}, status_code=500
        )
...
async def retrain_avni_analyzer_request(request: Request) -> JSONResponse:
    ...
    try:
        ...
        return JSONResponse({"error": f"Retraining failed: {str(e)}"}, status_code=500)
