...
def validate_metadata(file_path: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        if metadata["priority"] not in VALID_PRIORITIES:
            return (
                False,
                f"Invalid priority '{metadata['priority']}'. Must be one of: {', '.join(VALID_PRIORITIES)}",
            )
        ...
        if metadata["difficulty"] not in VALID_DIFFICULTIES:
            return (
                False,
                f"Invalid difficulty '{metadata['difficulty']}'. Must be one of: {', '.join(VALID_DIFFICULTIES)}",
            )
        ...
        if metadata["audience"] not in VALID_AUDIENCES:
            return (
                False,
                f"Invalid audience '{metadata['audience']}'. Must be one of: {', '.join(VALID_AUDIENCES)}",
            )
...
def scan_directory(directory: str, exclude_dirs: List[str] = None) -> dict:
    ...
    results = {"valid": [], "invalid": [], "no_metadata": []}
    ...
    md_files = [f for f in md_files if not any(excl in f for excl in exclude_dirs)]
...
def print_results(results: dict):
    ...
    total = (
        len(results["valid"]) + len(results["invalid"]) + len(results["no_metadata"])
    )
